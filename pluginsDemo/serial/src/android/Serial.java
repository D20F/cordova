				...
				while (true) {
					...
					try {
						...
						if (mInputStream != null) {
							int len;
							while (-1 != (len = mInputStream.read())) {
								Log.d(TAG, "data_len: " + len + ", size:"+size);
								if(size < buffer.length){
									buffer[size] = (byte) len;
									
									if (len == 126) {
										receiveMgs = toHexString(buffer, size);
	...
	private void openSerial(final JSONObject opts, final CallbackContext callbackContext) {
		cordova.getThreadPool().execute(new Runnable() {
			public void run() {
				try {
					if(mSerialPort == null){
						String path = opts.has("path") ? opts.getString("path") : "/dev/ttyMT0";
